import { Globe2, ShieldCheck, Users } from "lucide-react";
import Reveal from "./Reveal";

const STATS = [
  {
    icon: Globe2,
    value: "7+",
    label: "Ülkeden müşteri deneyimi",
  },
  {
    icon: Users,
    value: "1000+",
    label: "Tamamlanan işlem",
  },
  {
    icon: ShieldCheck,
    value: "%100",
    label: "Gizlilik ve güvenilirlik",
  },
];

export default function About() {
  return (
    <section id="hakkimizda" className="relative bg-cream py-24 sm:py-32">
      <div className="mx-auto max-w-6xl px-6">
        <div className="grid gap-16 lg:grid-cols-[1.1fr_0.9fr] lg:gap-20">
          <div>
            <Reveal>
              <span className="text-xs font-semibold uppercase tracking-widest text-gold">
                Hakkımızda
              </span>
              <h2 className="mt-4 text-3xl font-semibold text-navy sm:text-4xl text-balance">
                Uluslararası müşterilerin güvendiği bir danışmanlık ortağı
              </h2>
            </Reveal>

            <Reveal delay={0.1}>
              <p className="mt-6 text-base leading-relaxed text-navy/70 sm:text-lg">
                Global Consulting Group olarak Letonya, Litvanya, Estonya,
                Fransa, Almanya, Rusya ve Ukrayna başta olmak üzere farklı
                ülkelerden müşterilerle uzun soluklu çalışma deneyimine
                sahibiz. Türkiye&apos;de yaşamayı, çalışmayı ya da yatırım
                yapmayı planlayan müşterilerimize; tercüme, noter işlemleri ve
                ikamet/göç danışmanlığı alanlarında uçtan uca destek
                sunuyoruz.
              </p>
            </Reveal>

            <Reveal delay={0.18}>
              <p className="mt-4 text-base leading-relaxed text-navy/70 sm:text-lg">
                Her dosyayı özenle takip ediyor, resmi kurumlarla iletişimi
                sizin adınıza yönetiyor ve süreç boyunca şeffaf bilgi akışı
                sağlıyoruz. Amacımız, karmaşık bürokratik süreçleri sizin için
                basit ve güvenilir hale getirmek.
              </p>
            </Reveal>

            <Reveal delay={0.26}>
              <div className="mt-10 grid grid-cols-3 gap-4 sm:gap-8">
                {STATS.map(({ icon: Icon, value, label }) => (
                  <div key={label} className="flex flex-col gap-2">
                    <Icon className="text-gold" size={22} />
                    <span className="text-2xl font-semibold text-navy sm:text-3xl">
                      {value}
                    </span>
                    <span className="text-xs text-navy/60 sm:text-sm">
                      {label}
                    </span>
                  </div>
                ))}
              </div>
            </Reveal>
          </div>

          <Reveal delay={0.15} className="relative">
            <div className="relative overflow-hidden rounded-3xl bg-navy p-10 shadow-2xl shadow-navy/20 sm:p-12">
              <div className="absolute -right-10 -top-10 h-40 w-40 rounded-full bg-gold/10 blur-2xl" />
              <div className="absolute -bottom-10 -left-10 h-40 w-40 rounded-full bg-gold/10 blur-2xl" />
              <p className="relative font-serif text-2xl italic leading-relaxed text-white/90 sm:text-3xl">
                &ldquo;Farklı kültürlerden gelen müşterilerimize aynı özen ve
                güvenle hizmet veriyoruz.&rdquo;
              </p>
              <div className="relative mt-8 h-px w-16 bg-gold" />
              <p className="relative mt-6 text-sm font-medium tracking-wide text-gold">
                GLOBAL CONSULTING GROUP
              </p>
              <p className="relative text-sm text-white/50">
                Manavgat, Antalya
              </p>
            </div>
          </Reveal>
        </div>
      </div>
    </section>
  );
}
